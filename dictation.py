...
def audio_peak(audio):
    ...
    if audio.ndim > 1:
        audio = audio[:, 0]
    # int16 abs(-32768) overflows back to -32768; widen before abs
...
def mic_capture_broken(audio):
    """PipeWire glitches can stream constant -32768 or silence — not real speech."""
    if audio is None or audio.size == 0:
        return True
...
def meter_peak(audio):
    """Level bar only — stuck streams must not show 100%."""
...
class DictationApp:
    ...
    def _start_recording(self):
        ...
        if not self._ensure_mic_ready():
            ...
            return
        if time.time() - self._last_record_stop_at < 0.6:
            self._set_status("停录后请稍等半秒再开始", ORANGE, hold_sec=1.5)
            return
    ...
    def _on_rt_connect_failed(self, msg, rt_token):
        ...
        if (
            ...
            return
        self.status_label.config(text=friendly_connect_error(msg), fg="#ff6b6b")
...
def cli_test_ui_e2e():
    ...
    app._committed_text = "E2E按钮测试\n"
    app._copy_all()
    root.update()
    try:
        clip = root.clipboard_get()
        check("copy_all", "E2E" in clip)
    except tk.TclError:
    ...
    check("auto_copy_off", not app.auto_copy.get())
    app.auto_copy.set(True)
    check("auto_copy_on", app.auto_copy.get())
    ...
    try:
        ...
        check("copy_live", "实时区测试" in clip2)
    except tk.TclError:
    ...
    if fails:
        ...
        return 1
    print("OK: pin, copy_all, copy_live, clear, mode switch, auto_copy")
    return 0
